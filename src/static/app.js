    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0 
          ? details.participants.map((p, idx) => `<li><span class="participant-email">${p}</span><button class="delete-btn" data-activity="${name}" data-email="${p}" data-index="${idx}" title="Remove participant">✕</button></li>`).join('')
